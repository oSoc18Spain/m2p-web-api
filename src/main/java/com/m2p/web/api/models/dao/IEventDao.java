...
public interface IEventDao extends CrudRepository<Event, Long> {
	...
	@Query(value = "select ev.* from event ev join linechannel lc on lc.id = ev.linechannel_id where ev.state_event like %?2% and ev.linechannel_id = ?1", nativeQuery = true)
	public List<Event> findByLinexStatus(Long lineaId, String status);
	
	@Query(value = "select ev.* from event ev where ev.userassigned_id = ?1", nativeQuery = true)
